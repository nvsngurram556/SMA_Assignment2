...
U_DATA_PATH = os.path.join(DATA_DIR, "ml-100k", "u.data")
...
def download_and_extract_if_needed():
    if os.path.exists(U_DATA_PATH):
        print(f"Dataset already present at {U_DATA_PATH}; skipping download.")
        return
    ...
    cmd = "kaggle datasets download -d prajitdatta/movielens-100k-dataset -p ./data"
    ...
    for fname in os.listdir(DATA_DIR):
        if fname.endswith(".zip") and "movielens-100k-dataset" in fname:
...
def save_influencers(pr_scores, G, top_k=25):
    ...
    full_path = os.path.join(DIFFUSION_DIR, 'pagerank_scores.csv')
    df_scores.to_csv(full_path, index=False)
    print(f"All PageRank scores saved to: {full_path}")

    top_path = os.path.join(DIFFUSION_DIR, f'pagerank_top_{top_k}_influencers.csv')
...
def visualize_pagerank(G, pr_scores, max_nodes=150):
    ...
    img_path = os.path.join(DIFFUSION_DIR, 'pagerank_top_nodes.png')
...
def visualize_pagerank_diffusion(G, pr_scores, steps=5):
    ...
    for i in range(steps):
        ...
        plt.axis('off')
        img_path = os.path.join(DIFFUSION_DIR, f'pagerank_diffusion_step_{i+1}.png')
        plt.savefig(img_path, dpi=180, bbox_inches='tight')
        plt.close()
...
def create_diffusion_gif(steps=5):
    ...
    if images:
        gif_path = os.path.join(DIFFUSION_DIR, 'pagerank_diffusion.gif')
...
if __name__ == "__main__":
    ...
    if not os.path.exists(U_DATA_PATH):
        print(f"ERROR: Expected ratings file not found at {U_DATA_PATH}")
        sys.exit(1)
    ratings = load_movielens_u_data(U_DATA_PATH)
    ...
    # Save results
    save_graphml(G, os.path.join(DIFFUSION_DIR, 'pagerank_user_user_graph.graphml'))
    ...
    community_txt = os.path.join(COMMUNITY_DIR, 'community_partition.txt')
    ...
    community_graphml = os.path.join(COMMUNITY_DIR, 'community_subgraph.graphml')
    nx.write_graphml(subgraph, community_graphml)
    print(f"Community subgraph saved to: {community_graphml}")

    # Visualization
    community_png = os.path.join(COMMUNITY_DIR, 'community_subgraph.png')
    ...
    # Histograms
    plot_histogram(ratings['user_id'], 'User ID', 'Count', 'Distribution of Users', os.path.join(RECOMMENDER_DIR, 'hist_users.png'))
    plot_histogram(ratings['item_id'], 'Item ID', 'Count', 'Distribution of Items', os.path.join(RECOMMENDER_DIR, 'hist_items.png'))
    plot_histogram(ratings['rating'], 'Rating', 'Count', 'Distribution of Ratings', os.path.join(RECOMMENDER_DIR, 'hist_ratings.png'))
    ...
    rec_csv_path = os.path.join(RECOMMENDER_DIR, 'recommendations.csv')
    ...
    eval_path = os.path.join(RECOMMENDER_DIR, 'evaluation.txt')
